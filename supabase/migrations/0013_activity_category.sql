-- =============================================================================
-- Add a `category` to activities so the founder can distinguish teaching from
-- dawah/community work from non-counted time (travel, admin, construction
-- supervision, etc.). Updates the admin rollup to expose counted_hours and
-- teaching_hours for the monthly totals page.
-- Safe to re-run.
-- =============================================================================

alter table activities
  add column if not exists category text not null default 'teaching'
    check (category in ('teaching', 'dawah', 'umrah_teaching', 'other'));

create index if not exists idx_activities_category on activities(category);

-- -----------------------------------------------------------------------------
-- Rollup update: return counted_hours (teaching + dawah + umrah_teaching)
-- and teaching_hours (teaching only) alongside the existing totals.
-- -----------------------------------------------------------------------------
